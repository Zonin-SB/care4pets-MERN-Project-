...
import { expertAccepted, getExpertDetails } from '../../Axios/Services/ExpertServices';

function ExpertAcceptedNotificationPage() {
   ...
   async function fetchData() {
     ...
     setExpertDetails(data.expertDetails.accepted[0]);
   }
   }, [id])

   async function expertAcceptedMessage(id){
    const token =localStorage.getItem('expertToken')
    const data = await expertAccepted(token, id);
    if(data.status==='ok'){
      ...
      <div className="card px-8 py-4">
        ...
        {error?<p style={{color:'red'}} className="text-center">{error}</p> : ''}
          ...
          <p className="text-base md:text-lg text-gray-700 mt-3 ">
            ...
            <br />
            By
            <p className='font-bold'>Care4pets</p>
          </p>
        </div>
        <div className="card-mail flex items-center my-10">
         
          <button onClick={()=>expertAcceptedMessage(expertData.expertId)} className="bg-orange-500 hover:bg-orange-600 hover:border-orange-600 text-white font-bold capitalize px-3 py-2 text-base md:text-lg rounded-md border-t border-r border-b border-orange-500">
            Ok
          </button>
        </div>
      </div>
...
export default ExpertAcceptedNotificationPage
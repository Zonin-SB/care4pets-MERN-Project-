...
import { expertRejectionAccepted, getExpertDetails } from '../../Axios/Services/ExpertServices';

function ExpertRejectedReasonPage() {
    const navigate=useNavigate();
    const [expertDetails, setExpertDetails] = useState([]);
    const [error,setError]=useState('')
    const { id } = useParams();
    const expertData = useSelector((state) => state.admin.expertDetails);
   ...
   async function fetchData() {
     const data = await getExpertDetails(token, id);

     setExpertDetails(data.expertDetails.rejected[0]);
   }
   }, [id])

   async function expertRejected(id){
    const token =localStorage.getItem('expertToken')
    const data = await expertRejectionAccepted(token, id);
    if(data.status==='ok'){
      ...
      <div className="card px-8 py-4">
        ...
        {error?<p style={{color:'red'}} className="text-center">{error}</p> : ''}
          <h1 className="text-xl md:text-2xl font-bold leading-tight text-gray-900">
            Hello {expertData.name}
          </h1>
          <p className="text-base md:text-lg text-gray-700 mt-3 ">
            It is to inform you that your application was rejected.
            <br />
            Reason : {expertDetails.reason}
            <br />
            Message : {expertDetails.message}
          </p>
        </div>
        <div className="card-mail flex items-center my-10">
         
          <button onClick={()=>expertRejected(expertData.expertId)} className="bg-orange-500 hover:bg-orange-600 hover:border-orange-600 text-white font-bold capitalize px-3 py-2 text-base md:text-lg rounded-md border-t border-r border-b border-orange-500">
            Ok
          </button>
        </div>
      </div>
...
export default ExpertRejectedReasonPage
...
import { getExpertChangeNotification, userExpertRejectionAccepted } from '../../Axios/Services/UserServices';

function UserAcceptExpChangeRejection() {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
         const data=await getExpertChangeNotification(token,id)
         setNotification(data.details.expertChangeRejected[0])
        }
        }, [id])

        async function userAccepted(id){
            const token =localStorage.getItem('userToken')
            const data = await userExpertRejectionAccepted(token, id);
            if(data.status==='ok'){
    ...
    <div className="wrapper bg-white rounded-sm shadow-lg">
      <div className="card px-8 py-4">
        ...
        {error?<p style={{color:'red'}} className="text-center">{error}</p> : ''}
          ...
          <p className="text-base md:text-lg text-gray-700 mt-3 ">This is to inform you that your application for expert change has been rejected.</p>
          <p className="text-base md:text-lg text-gray-700 mt-3 ">
           
           
            Reason : {notification.reason?notification.reason:''}
            <br />
        ...
        <div className="card-mail flex items-center my-10">
          ...
          <button
           onClick={()=>userAccepted(notification.id)}
...
export default UserAcceptExpChangeRejection
...
import { getExpertChangeNotification, userExpertChangeAccepted } from '../../Axios/Services/UserServices';

function UserAcceptExpChangeApproved() {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
         ...
         setNotification(data.details.expertChangeAccepted[0])
        }
        }, [id])

        async function userAccepted(id){
            const token =localStorage.getItem('userToken')
            const data = await userExpertChangeAccepted(token, id);
            if(data.status==='ok'){
...
export default UserAcceptExpChangeApproved